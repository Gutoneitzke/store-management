    ...
    <AppLayout title="Editar cliente">
        <PageCard>
            <div class="flex gp-2 items-center justify-between">
                <h1 class="text-2xl">Editando cliente: {{ customer.name }}</h1>
...
export default {
    ...
    props: ['customer','locales','myStores'],
    data() {
        return {
            form: {
    ...
    mounted(){
        ...
        all.forEach(a => {
            Object.values(a.states).forEach(s => {
                s.cities.forEach(c => {
                    if(c.id == this.customer.cities_id){
                        this.form.country = a;
                        this.form.state = s;
                    }
                })
